...
(HubCluster hub + CpuCluster spoke) in a sequential phase-driven test plan.
...
Usage:
    python3 test/soak/multi_cluster_test.py \
        --fleet-url https://fleet-controller.apps.hubcluster.example.com \
...
CLUSTERS = [
    ...
    {"id": "gpucluster-h100", "name": "GpuCluster H100 NVL", "region": "us-east-1",
     "labels": {"gpu": "nvidia-h100", "runtime": "vllm", "hardware": "granite-rapids"}},
]

PRAXIS_URL = os.environ.get("PRAXIS_URL", "https://praxis.example.com")

EXTERNAL_INFERENCE_URLS = {
    "hubcluster-sno": PRAXIS_URL,
    "cpucluster-xeon6": PRAXIS_URL,
    "gpucluster-h100": PRAXIS_URL,
}
...
class MultiClusterTest:
    def __init__(self, fleet_url: str, ledger_url: str = "",
                 cpucluster_cluster_id: str = "cpucluster-xeon6",
                 hubcluster_cluster_id: str = "hubcluster-sno",
                 timeout: float = 30.0,
                 gcl_signing_key: str = "",
                 gcl_key_id: str = "",
                 token: str = ""):
        ...
        self.cpucluster_id = cpucluster_cluster_id
        self.hubcluster_id = hubcluster_cluster_id
    ...
    async def phase_cluster_registration(self, state: SoakState):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if self.cpucluster_id in cluster_ids and self.hubcluster_id in cluster_ids:
                    cap.successes += 1
                    cap.latencies.append(ms)
                else:
                    cap.errors += 1
                    cap.last_error = (
                        f"missing clusters: want [{self.cpucluster_id}, {self.hubcluster_id}], "
                        f"got {cluster_ids}"
                    )
            else:
                cap.errors += 1
    ...
    async def phase_cross_cluster_routing(self, state: SoakState):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if self.cpucluster_id in cluster_ids and self.hubcluster_id in cluster_ids:
                    cap.successes += 1
                    state.routing_cluster_hits[self.cpucluster_id] = \
                        state.routing_cluster_hits.get(self.cpucluster_id, 0) + 1
                    state.routing_cluster_hits[self.hubcluster_id] = \
                        state.routing_cluster_hits.get(self.hubcluster_id, 0) + 1
                else:
                    cap.errors += 1
            ...
            for cid in [self.cpucluster_id, self.hubcluster_id]:
                metrics = {
                    "cluster_id": cid,
    ...
    async def phase_failover(self, state: SoakState):
        ...
        try:
            # Drain cpucluster
            resp, ms = await self._req(
                "POST", f"{self.fleet}/api/v1/clusters/{self.cpucluster_id}/drain", {})
            if resp.status_code == 200:
                cap.successes += 1
                cap.latencies.append(ms)
            else:
                cap.errors += 1
                cap.last_error = f"drain cpucluster: {resp.status_code}"

            # Verify cpucluster status is Draining
            resp, ms = await self._req(
            ...
            if resp.status_code == 200:
                ...
                cpucluster = next(
                    (c for c in clusters
                     if c.get("id", c.get("ID", "")) == self.cpucluster_id), None)
                if cpucluster and cpucluster.get("phase", cpucluster.get("status", "")) in (
                        "Draining", "draining"):
                    cap.successes += 1
                else:
                    cap.errors += 1
                    cap.last_error = f"cpucluster not draining: {cpucluster}"
            else:
                cap.errors += 1
                cap.last_error = f"list clusters: {resp.status_code}"

            # Verify cpucluster is excluded from fleet metrics (draining)
            resp, ms = await self._req("GET", f"{self.fleet}/api/v1/clusters")
            if resp.status_code == 200:
                cap.latencies.append(ms)
                clusters = resp.json() if isinstance(resp.json(), list) else []
                ...
                if self.cpucluster_id not in running:
                    cap.successes += 1
                else:
                    cap.errors += 1
                    cap.last_error = "cpucluster still Running after drain"
                if self.hubcluster_id in [c["id"] for c in clusters]:
                    cap.successes += 1
                else:
                    cap.errors += 1
                    cap.last_error = "hubcluster missing after drain"
    ...
    async def phase_drain_activate_cycle(self, state: SoakState):
        cap = state.phase(5)
        try:
            # Reactivate cpucluster
            resp, ms = await self._req(
                "POST", f"{self.fleet}/api/v1/clusters/{self.cpucluster_id}/activate", {})
            if resp.status_code == 200:
                cap.successes += 1
                cap.latencies.append(ms)
            else:
                cap.errors += 1
                cap.last_error = f"activate cpucluster: {resp.status_code}"

            # Verify cpucluster is Running
            resp, ms = await self._req("GET", f"{self.fleet}/api/v1/clusters")
            if resp.status_code == 200:
                clusters = resp.json() if isinstance(resp.json(), list) else []
                cpucluster = next(
                ...
                if cpucluster and cpucluster.get("phase", cpucluster.get("status", "")) in (
                    ...
                    cap.errors += 1
                    cap.last_error = f"cpucluster not running: {cpucluster}"
            else:
                cap.errors += 1
                cap.last_error = f"list clusters: {resp.status_code}"

            # Now drain hubcluster
            resp, ms = await self._req(
                "POST", f"{self.fleet}/api/v1/clusters/{self.hubcluster_id}/drain", {})
            if resp.status_code == 200:
                cap.successes += 1
                cap.latencies.append(ms)
            else:
                cap.errors += 1
                cap.last_error = f"drain hubcluster: {resp.status_code}"

            # Verify hubcluster is draining, cpucluster is running
            resp, ms = await self._req("GET", f"{self.fleet}/api/v1/clusters")
            if resp.status_code == 200:
                cap.latencies.append(ms)
                clusters = resp.json() if isinstance(resp.json(), list) else []
                statuses = {c["id"]: c.get("status", c.get("phase", "")) for c in clusters}
                if statuses.get(self.hubcluster_id) in ("Draining", "draining", "Drained", "Degraded"):
                    cap.successes += 1
                else:
                    cap.errors += 1
                    cap.last_error = f"hubcluster not draining: {statuses.get(self.hubcluster_id)}"
                if statuses.get(self.cpucluster_id) in ("Running", "running", "Healthy", "Active", "Degraded"):
                    cap.successes += 1
                else:
                    cap.errors += 1
                    cap.last_error = f"cpucluster not running: {statuses.get(self.cpucluster_id)}"

            # Reactivate hubcluster
            resp, ms = await self._req(
                "POST", f"{self.fleet}/api/v1/clusters/{self.hubcluster_id}/activate", {})
            if resp.status_code == 200:
                cap.successes += 1
                cap.latencies.append(ms)
            else:
                cap.errors += 1
                cap.last_error = f"reactivate hubcluster: {resp.status_code}"
    ...
    async def phase_session_affinity(self, state: SoakState):
        ...
        try:
            ...
            bound_cluster = self.cpucluster_id
            ...
            if resp.status_code == 200:
                ...
                if cpucluster and cpucluster.get("status") in ("Draining", "draining", "Drained", "Degraded"):
                    cap.successes += 1
                else:
                    cap.errors += 1
                    cap.last_error = f"expected draining: {cpucluster}"
            else:
                cap.errors += 1
    ...
    async def phase_autoscaling_signals(self, state: SoakState):
        cap = state.phase(8)
        try:
            # CpuCluster: high load signals
            cpucluster_metrics = {
                "cluster_id": self.cpucluster_id,
                "throughput_tps": 5.0,
                "ttft_p50_ms": 200.0,
                "ttft_p99_ms": 800.0,
                "queue_depth": 85,
            ...
            resp, ms = await self._req(
                "POST", f"{self.fleet}/api/v1/agent/metrics", cpucluster_metrics)
            if resp.status_code == 202:
                cap.successes += 1
                cap.latencies.append(ms)
            else:
                cap.errors += 1
                cap.last_error = f"cpucluster metrics: {resp.status_code}"

            # HubCluster: low utilization signals
            hubcluster_metrics = {
                "cluster_id": self.hubcluster_id,
                "throughput_tps": 50.0,
                "ttft_p50_ms": 15.0,
                "ttft_p99_ms": 40.0,
                "queue_depth": 2,
            ...
            resp, ms = await self._req(
                "POST", f"{self.fleet}/api/v1/agent/metrics", hubcluster_metrics)
            if resp.status_code == 202:
                cap.successes += 1
                cap.latencies.append(ms)
            else:
                cap.errors += 1
                cap.last_error = f"hubcluster metrics: {resp.status_code}"
    ...
    async def phase_observability_federation(self, state: SoakState):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if self.cpucluster_id in clusters_in_metrics and \
                        self.hubcluster_id in clusters_in_metrics:
                    cap.successes += 1
    ...
    async def phase_kv_cache_transfer(self, state: SoakState):
        ...
        try:
            params = {
                ...
                "source_cluster": self.hubcluster_id,
                "target_cluster": "gpucluster-h100",
            ...
            if self.gcl_signing_key and self.gcl_key_id:
                ...
                intent = {"type": "kv_transfer",
                          ...
                          "target_clusters": [params.get("target_cluster", self.hubcluster_id)]}
                resp, ms = await self._req(
        ...
        try:
            ...
            if self.gcl_signing_key and self.gcl_key_id:
                ...
                intent = {
                    ...
                    "target_clusters": params.get("clusters", [self.hubcluster_id]),
                }
                resp, ms = await self._req(
                    "POST", f"{self.fleet}/api/v1/intents", intent,
                    {"Content-Type": "application/json"})
    ...
    async def phase_live_inference_cpu(self, state: SoakState):
        cap = state.phase(14)
        try:
            cpu_ids = [self.hubcluster_id, self.cpucluster_id]
    ...
    async def phase_live_inference_gpu(self, state: SoakState):
        cap = state.phase(15)
        try:
            inference_url = EXTERNAL_INFERENCE_URLS.get("gpucluster-h100", "")
            if not inference_url:
                cap.errors += 1
                cap.last_error = "no external URL for gpucluster-h100"
                return
    ...
    async def phase_fleet_orchestrated_inference(self, state: SoakState):
        ...
        try:
            ...
            is_gpu = target_id == "gpucluster-h100"
            ...
            if route_resp.status_code == 200:
                ...
                if routed_id in EXTERNAL_INFERENCE_URLS:
                    ...
                    is_gpu = routed_id == "gpucluster-h100"
    ...
    async def phase_three_cluster_grid_failover(self, state: SoakState):
        ...
        try:
            ...
            spoke_id = None
            for c in clusters:
                if c.get("id") not in ("hubcluster",) and c.get("status") == "Running":
                    spoke_id = c["id"]
                    break
    ...
    def _build_json_output(self, state: SoakState, slo_pass: bool) -> dict:
        ...
        return {
            ...
            "clusters": {
                "cpucluster": self.cpucluster_id,
                "hubcluster": self.hubcluster_id,
            },
            "phases": phases_out,
...
async def main():
    ...
    parser.add_argument("--fleet-url", required=True,
                        help="HubCluster fleet controller Route URL")
    parser.add_argument("--ledger-url", default="",
                        help="Ledger gateway URL (optional)")
    parser.add_argument("--cpucluster-cluster-id", default="cpucluster-xeon6",
                        help="CpuCluster spoke cluster ID")
    parser.add_argument("--hubcluster-cluster-id", default="hubcluster-sno",
                        help="HubCluster hub cluster ID")
    ...
    test = MultiClusterTest(
        ...
        cpucluster_cluster_id=args.cpucluster_cluster_id,
        hubcluster_cluster_id=args.hubcluster_cluster_id,
