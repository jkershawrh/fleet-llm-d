...
def test_evaluate_passes_only_with_complete_clean_evidence():
    report = MODULE.evaluate(traffic(), [resource("oberon"), resource("arena")], {"oberon", "arena"}, 70)
    assert report["passed"] is True
    assert report["failures"] == []


def test_evaluate_fails_closed_for_missing_or_unhealthy_provider():
    report = MODULE.evaluate(traffic(), [resource("oberon", restarts=1)], {"oberon", "arena"}, 70)
    assert report["passed"] is False
    assert "missing resource evidence: arena" in report["failures"]
    assert "oberon recorded container restarts" in report["failures"]


def test_evaluate_rejects_traffic_errors_and_guardrail_breach():
    report = MODULE.evaluate(traffic(errors=1), [resource("oberon", memory=70)], {"oberon"}, 70)
    assert report["passed"] is False
    assert "traffic level 0 recorded errors" in report["failures"]
    assert "oberon reached the 70% resource guardrail" in report["failures"]
