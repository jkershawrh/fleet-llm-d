...
resp, wall = call_model(
    [{"role": "system", "content": "You are a fleet infrastructure assistant. Use the available tools."},
     {"role": "user", "content": "Check the status of the gpucluster cluster and include metrics"}],
    300, FLEET_TOOLS)
p = parse(resp)
tc = p["tool_calls"]
tool = tc[0]["name"] if tc else "none"
valid = "yes" if tc and tc[0].get("args", {}).get("cluster_name") == "gpucluster" and tc[0]["args"].get("include_metrics") == True else "no"
...
resp, wall = call_model(
    [{"role": "system", "content": "You are a fleet infrastructure assistant. Use the available tools."},
     {"role": "user", "content": "CpuCluster cluster is at 95% GPU utilization and we have maintenance in 2 hours. What should we do?"}],
    500, FLEET_TOOLS)
p = parse(resp)
tc = p["tool_calls"]
...
reasoning_tests = [
    ("Diagnose fleet latency",
     "Our fleet has 3 clusters: hubcluster (CPU, 10 RPS), cpucluster (CPU, 8 RPS), gpucluster (H100 GPU, 80 RPS). "
    ...
    ("Incident triage",
     "Alert: cluster gpucluster health check failed 3 consecutive times. fleet-agent last reported 2 min ago. "
     "GPU utilization was 94% before contact lost. 5 active inference sessions on gpucluster. "
     "Failover chain: gpucluster -> cpucluster -> hubcluster. Immediate actions in priority order?",
     ["failover", "drain", "redirect", "cpucluster", "session"]),
]
...
resp, wall = call_model(
    [{"role": "system", "content": "You are a Kubernetes expert. Output only valid YAML, no explanation."},
     {"role": "user", "content": "Generate a FleetCluster custom resource YAML for cluster named gpucluster "
      "with labels gpu-type=H100, region=us-east, capacity=94GB. "
      "Endpoint https://api.gpucluster.local:6443, health checks every 30s."}], 500)
p = parse(resp)
valid = "yes" if "FleetCluster" in p["answer"] or "kind:" in p["answer"] else "no"
fields = "yes" if all(x in p["answer"] for x in ["gpucluster", "H100", "6443"]) else "no"
...
resp, wall = call_model(
    [{"role": "system", "content": "Extract the requested information as a JSON object. Output only valid JSON."},
     {"role": "user", "content": "From this log line, extract cluster name, error type, and timestamp as JSON:\n\n"
      ...
      "on cluster cpucluster-prod-eu: insufficient GPU memory (requested 80GB, available 24GB)"}], 300)
p = parse(resp)
valid_json = "no"
fields_ok = "no"
try:
    ...
    if json_match:
        ...
        valid_json = "yes"
        if "cpucluster" in str(d).lower() and ("gpu" in str(d).lower() or "memory" in str(d).lower()):
            fields_ok = "yes"
except:
    pass
...
resp, wall = call_model(
    [{"role": "system", "content": "Output only valid JSON."},
     {"role": "user", "content": "Generate a GCL DecisionPackage JSON for scaling granite-3.1-8b-instruct "
      "from 2 to 4 replicas on cluster cpucluster due to queue depth exceeding threshold. "
...
try:
    ...
    if json_match:
        d = json.loads(json_match.group())
        valid_json = "yes"
        if d.get("target_replicas") == 4 and "cpucluster" in str(d).lower():
            fields_ok = "yes"
except:
    pass
...
resp, wall = call_model(
    [{"role": "system", "content": "Write code only. No explanation."},
     {"role": "user", "content": "Write a bash script that checks health of 3 fleet clusters "
      "(hub, CPU provider, and GPU provider) "
