...
|-------------|--------|-------|
| HMAC bearer token issuance and validation for protected REST endpoints | Done | `pkg/auth/token.go` implements the project-specific HMAC-SHA256 token format (not JWT); refresh uses `handleRefreshToken`; healthz/readyz/metrics are exempt. Production still needs OIDC or workload identity. |
...
| Requirement | Status | Notes |
|-------------|--------|-------|
| RBAC enforcement for all 12 shipped CRDs | Done | 4 ClusterRoles in `deploy/kustomize/base/rbac.yaml` (controller, agent, viewer, tenant-admin), including the two Praxis Grid integration resources. |
...
| Requirement | Status | Notes |
|-------------|--------|-------|
| TLS in transit for production-configured channels | In Progress | Controller supports `--tls-cert`/`--tls-key`; fleetctl supports `--tls-ca`; fleet-agent supports pinned CA verification through `--tls-ca-cert` / `FLEET_TLS_CA_CERT`; production startup requires PostgreSQL `sslmode=verify-full` and HTTPS Praxis/ledger endpoints. Remaining gaps: certificate provisioning/rotation is operator-owned, mutual workload identity is absent, and KV transfer TCP has no TLS. |
...
| PII and sensitive data handling in inference logs | Done | The Go controller reads inference bodies for classification and forwarding but `RequestLoggingMiddleware` restricts logs to method, path, status, and latency and prohibits body/header/query logging. The Rust agent proxy logs operational metadata and body size, not prompt contents. Remaining gaps: runtime debug logging needs governance and there is no application-level retention policy. |
