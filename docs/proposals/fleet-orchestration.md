...
- **Telco Provider A** requires a multi-cluster mesh across 30+ edge sites with
...
- **Telco Provider B** asks for a single pane of glass across their inference fleet
  with per-tenant cost controls and chargeback to internal business units.
- **Mobile Network Provider** selected NVIDIA NVAIE + Rafay specifically for tenant
...
- **Financial Services Provider A** runs a 5-model production deployment requiring
...
- **Financial Services Provider B** named multi-cluster routing as their stated top
...
- **NVIDIA NVAIE + Rafay**: The combination of NVIDIA AI Enterprise's GPU
  ...
  tenant self-service and GPU fleet management. This is the stack Mobile Network Provider
...
|---|---|---|---|
| Telco Provider A | Telco AI Grid (30+ edge sites) | PlacementPolicy, FleetRoutingPolicy, TenantProfile, FleetInferencePool | Architecture review |
| Financial Services Provider A | Regulated financial services | All 7 CRDs | Design partner |
...
| Mobile Network Provider | Tenant self-service | TenantProfile, FleetInferencePool | Competitive loss analysis |
